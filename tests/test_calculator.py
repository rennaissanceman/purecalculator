...
def test_finite_or_none_returns_value_for_finite():
    assert src.calculator._finite_or_none(3.14, "test") == 3.14


def test_finite_or_none_returns_none_and_warns_on_inf():
    with pytest.warns(UserWarning, match="non-finite"):
        assert src.calculator._finite_or_none(float("inf"), "add") is None


def test_finite_or_none_returns_none_and_warns_on_nan():
    with pytest.warns(UserWarning, match="non-finite"):
        val = src.calculator._finite_or_none(float("nan"), "mul")
        assert val is None
...
def test_add_normal(x, y, expected):
    c = src.calculator.Calculator(x, y)
...
def test_add_overflow_goes_to_warning_and_none():
    c = src.calculator.Calculator(float_info.max, float_info.max)
    with pytest.warns(UserWarning, match="non-finite"):
...
def test_multiply_normal():
    c = src.calculator.Calculator(6.0, 7.0)
...
def test_multiply_with_infinite_operand_warns_nonfinite():
    c = src.calculator.Calculator(float("inf"), 2.0)
    with pytest.warns(UserWarning, match="non-finite"):
        assert c.multiply() is None
...
def test_divide_by_nan_or_inf_warns_and_returns_none():
    for denom in (float("nan"), float("inf"), -float("inf")):
        c = src.calculator.Calculator(1.0, denom)
        with pytest.warns(UserWarning, match="near"):
            assert c.divide() is None


def test_divide_nonfinite_result_warns_and_none():
    c = src.calculator.Calculator(float_info.max, 1.0)
...
def test_actions_contract_has_expected_ops_and_methods_present():
    expected = {"1", "2", "3", "4"}
    assert set(src.calculator.ACTIONS.keys()) == expected
    for k, (_label, _sym, method) in src.calculator.ACTIONS.items():
        assert hasattr(src.calculator.Calculator, method), f"Missing method {method} for ACTION {k}"


def test_near_zero_tol_positive_and_small():
    assert src.calculator.NEAR_ZERO_TOL > 0
    assert src.calculator.NEAR_ZERO_TOL < 1e-6
...
def test_parse_number_accepts_dot_and_comma(monkeypatch):
    ...
    val = src.calculator._parse_number("Enter: ")
    assert val == 3.14
...
def test_ask_menu_returns_stripped_choice(monkeypatch, capsys):
    ...
    choice = src.calculator._ask_menu()
...
def test_run_action_valid_addition_prints_result(capsys):
    calc = src.calculator.Calculator(10, 5)
    res = src.calculator._run_action(calc, "1")
    out = capsys.readouterr().out
    assert res == 15
...
def test_run_action_invalid_choice_prints_message(capsys):
    calc = src.calculator.Calculator(1, 2)
    res = src.calculator._run_action(calc, "9")
    out = capsys.readouterr().out
    assert res is None
...
def test_run_action_missing_method_is_handled(monkeypatch, capsys):
    monkeypatch.setitem(src.calculator.ACTIONS, "9", ("Weird", "?", "not_implemented"))
    calc = src.calculator.Calculator(1, 2)
    res = src.calculator._run_action(calc, "9")
    out = capsys.readouterr().out
    assert res is None
...
def test_ask_quit_variants(monkeypatch, answer, expected):
    ...
    assert src.calculator._ask_quit() is expected
...
def test_main_handles_unexpected_exception(monkeypatch, capsys):
    ...
    monkeypatch.setattr(Calculator, "add", staticmethod(boom))
    # def boom(self):
    #     raise RuntimeError("boom")
